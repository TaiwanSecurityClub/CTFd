...
class ScoreboardDetail(Resource):
    ...
    @cache.cached(
        timeout=60,
        key_prefix=make_cache_key_with_query_string(allowed_params=["qualify", "bracket_id"]),
    )
    def get(self, count):
        response = {}
        ...
        standings = get_standings(count=count, bracket_id=bracket_id, qualify=qualify)
